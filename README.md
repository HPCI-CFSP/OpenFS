...
New research tasks and monitors should select topics from `config/research-baseline.json`. Revision 5 of `FSBASE-002` retains all 60 existing canonical Topic IDs and adds four IDs to separate AI and experimental/real-time workload needs from their evaluation. The 60 existing IDs comprise the protected 30-topic initial catalog, 27 additions from all 26 MEXT-linked FY2022-FY2025 PDFs, and three human-requested additions. There are now 64 historical IDs, including 24 retired entries with explicit successors, and 40 active Topics on Pages. No historical evidence or Consensus reference is renumbered. `config/catalog-taxonomy.json` assigns each active Topic and roadmap to exactly one of six categories; retired public codes remain reserved.

The [current catalog and migration table](docs/research-baseline/current-catalog.md) are generated from the bilingual scope definitions and `config/catalog-migration.json`. Each Topic has research units with explicit questions, progress, and evidence-section links; merging Topics does not mark uncovered units complete. Storage devices, software, and integrated products are investigated together under Architecture. Programming environments are consolidated, software sustainability is retained, and needs/evaluation pairs cover scientific simulation, AI, and experimental/real-time workloads. [Catalog maintenance](docs/operations/catalog-maintenance.md) defines ownership boundaries, legacy links, and ongoing research obligations.

An explicitly human-authorized, single-model investigation may use the [bounded provisional update procedure](docs/operations/provisional-research-updates.md). It pins the input commit and predecessor digests, appends evidence and read receipts, and detects concurrent edits. Results remain provisional; it cannot close Gaps, satisfy Consensus, or enable unattended production. The storage roadmap, procurement register, and AI-evaluation units include pilot updates under `DIR-900015`.

The manually maintained Watch Registry separates recurring official index and release pages from exact evidence documents. `tools/build_source_catalog_map.py` generates the URL-to-Topic/Roadmap/Track map from current registered public evidence. A changed Watch page is only a signal: automated adoption of a change requires confirmation in an exact primary source and the applicable Consensus Gate. Emerging-topic discovery can be initiated daily by `.github/workflows/emerging-topic-discovery.yml`; it remains fail-closed until the reviewed pilot, security-profile, budget, and Consensus-capacity gates are ready.
