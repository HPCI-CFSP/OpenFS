(function () {
  ...
  function searchItems() {
    ...
    data.topics.forEach((topic) => items.push({type: "topic", id: topic.topic_id, title_ja: topic.title_ja, title_en: topic.title_en, body_ja: profileMap.get(topic.topic_id)?.summary_ja || "", body_en: profileMap.get(topic.topic_id)?.summary_en || "", search: flatten([topic, profileMap.get(topic.topic_id)]), href: `${rootPrefix}?topic=${encodeURIComponent(topic.topic_id)}#catalog`, status: topic.verification_status}));
    data.roadmap_artifacts.forEach((roadmap) => {
      items.push({type: "roadmap", id: roadmap.roadmap_id, title_ja: roadmap.title_ja, title_en: roadmap.title_en, body_ja: roadmap.summary_ja, body_en: roadmap.summary_en, search: flatten(roadmap), href: `${rootPrefix}${roadmapPath(roadmap.roadmap_id)}`, status: roadmap.consensus_status});
