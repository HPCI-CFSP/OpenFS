(function () {
  ...
  function searchItems() {
    ...
    data.topics.forEach((topic) => { const category = categories.get(topic.catalog_category_id); items.push({type: "topic", id: topic.topic_id, title_ja: topic.title_ja, title_en: topic.title_en, body_ja: profileMap.get(topic.topic_id)?.summary_ja || "", body_en: profileMap.get(topic.topic_id)?.summary_en || "", search: flatten([topic, category, profileMap.get(topic.topic_id)]), href: `${rootPrefix}?topic=${encodeURIComponent(topic.topic_id)}#catalog`, status: topic.verification_status}); });
