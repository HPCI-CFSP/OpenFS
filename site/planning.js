(function () {
  ...
  const copy = {
    ...
    en: {
      ...
      gapQueueTitle: "P0 research assignment queue", gapQueueLead: "Assigns every P0 Gap to a weekly research Monitor or independent Consensus. Items under disabled Monitors remain explicitly staged for production readiness.", assignment: "Assignment", cadence: "Recheck cadence", executionState: "Execution state", queryPlan: "Query plan", weekly: "weekly", monthly: "monthly", quarterly: "quarterly", "continuous-until-quorum": "until quorum", "staged-monitor-disabled": "Monitor disabled; staged", "ready-for-scheduled-discovery": "scheduled discovery ready", "awaiting-independent-review": "awaiting independent review", "explicit-override": "explicit queries", "generated-fallback": "generated fallback", "not-applicable": "not applicable", assignedDiscovery: "Discovery assignments", assignedConsensus: "Consensus assignments", productionReady: "Scheduled-ready", p0ExplicitQueries: "P0 explicit queries", p0FallbackQueries: "P0 fallbacks",
  ...
  function renderEvidencePage() {
    const assurance = data.roadmap_assurance; const source = assurance.source_audit; const sourceTriage = assurance.source_triage; const evidence = assurance.evidence_audit; const freshness = assurance.freshness_audit; const gapQueue = assurance.gap_queue; const dependencies = assurance.dependency_register;
    ...
    [["evidence-audit-commit", evidence], ["freshness-audit-commit", freshness], ["gap-queue-commit", gapQueue], ["source-audit-commit", source], ["source-triage-commit", sourceTriage], ["dependency-register-commit", dependencies]].forEach(([id, artifact]) => { const link = document.getElementById(id); link.href = artifact.source_commit_url; link.textContent = `${id === "source-triage-commit" ? tr("retrievalTriage") : tr("commit")} ${artifact.source_commit.slice(0, 7)}`; });
