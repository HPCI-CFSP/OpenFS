(function () {
  ...
  function renderApplicationPerformance(roadmap) {
    ...
    const packageLabels = language === "ja"
      ? {status: "準備状況", code: "コード版", input: "入力版", license: "ライセンス", match: "EEA1入力との一致", harness: "実行基盤候補", codeInput: "コード・入力の固定候補", codeOnly: "コードのみ固定候補", blocked: "公開パッケージなし", notConfirmed: "未確認", noInput: "公開入力なし", codeLabel: "コード", inputLabel: "入力"}
      : {status: "Readiness", code: "Code version", input: "Input version", license: "License", match: "EEA1 input match", harness: "Harness candidate", codeInput: "Pinned code and input candidate", codeOnly: "Pinned code-only candidate", blocked: "No public package", notConfirmed: "Not confirmed", noInput: "No public input", codeLabel: "Code", inputLabel: "Input"};
    ...
    [tr("applicationColumn"), packageLabels.status, packageLabels.code, packageLabels.input, packageLabels.license, packageLabels.match, packageLabels.harness].forEach((label) => { const cell = document.createElement("th"); cell.textContent = label; packageHeadRow.append(cell); });
    ...
    performance.baseline_package_readiness.forEach((item) => {
      ...
      row.append(name, status, code, input, license, match, harness); packageBody.append(row);
    });
