(function () {
  ...
  const copy = {
    ja: {
      ...
      overallOutlook: "総合見通し（暫定）", outlookBasis: "公開情報の統合整理", planningYear: "システム計画年", planningYearSummary: "計画年までの製品化開始", availabilityConfirmed: "製品化・量産を確認", availabilityTarget: "製品化・量産の公表目標", availabilityUndisclosed: "製品化・量産時期未公表", availabilityTimelineNote: "主線表には製品化、量産、商用提供に関する確認済み実績または公式目標だけを表示します。選択した計画年以前の製品化開始は、その年の調達可能性、供給量、価格、保守期間を保証しません。研究、標準化、試作、サンプル、実証の根拠は項目を選択すると確認できます。", availabilityDetail: "製品化・量産の詳細", availabilityStatus: "製品化状況", availabilityType: "提供形態", commercialAvailability: "商用提供", commercialShipment: "商用出荷", volumeProduction: "量産", commercialization: "実用化", lifecycleEvidence: "製品化までの公開根拠", noLifecycleEvidence: "この段階の公開根拠は未確認", lifecycleResearch: "1. 構想・研究", lifecycleStandardization: "2. 標準化作業", lifecycleStandardPublished: "3. 規格・仕様公開", lifecyclePrototype: "4. 試作・サンプル", lifecyclePilot: "5. 実証・評価", sourcePublished: "発表日",
  ...
  const availabilityStatusKeys = {confirmed: "availabilityConfirmed", "announced-target": "availabilityTarget", "timing-undisclosed": "availabilityUndisclosed"};
  ...
  function availabilityButton(event) {
    const button = document.createElement("button");
    button.type = "button";
    button.className = `roadmap-milestone availability-${event.availability_status} precision-${event.timing_precision}`;
  ...
  function renderRoadmapLegend(roadmap) {
    ...
    if (roadmap.timeline_presentation === "market-availability") {
      ["confirmed", "announced-target", "timing-undisclosed"].forEach((status) => { const item = document.createElement("span"); item.className = `legend-item availability-${status}`; item.textContent = tr(availabilityStatusKeys[status]); root.append(item); });
      return;
    }
    ["commercial", "sample", "standard", "published", "target", "concept", "pilot", "decision-gate", "deployment", "undated"].forEach((maturity) => { const item = document.createElement("span"); item.className = `legend-item maturity-${maturity}`; item.textContent = tr(maturityKeys[maturity]); root.append(item); });
  }
  function renderAvailabilityControl(roadmap) {
    ...
    const count = document.createElement("strong"); count.textContent = `${tr("planningYearSummary")}: ${tr("availabilityConfirmed")} ${confirmed} / ${tr("availabilityTarget")} ${targets} / ${tr("availabilityUndisclosed")} ${undisclosed}`;
  ...
  function renderApplicationPerformance(roadmap) {
    ...
    const acceptanceMetricLabels = new Map(performance.acceptance_metric_catalog.map((item) => [item.metric_id, localized(item, "label")]));
    infrastructure.rows.forEach((matrixRow) => { const application = performance.applications.find((item) => item.application_id === matrixRow.application_id); const row = document.createElement("tr"); const name = document.createElement("th"); name.scope = "row"; const nameText = document.createElement("strong"); nameText.textContent = application?.name || matrixRow.application_id; const criteria = document.createElement("small"); criteria.textContent = matrixRow.planning_criterion_ids.map((id) => localized(planningCriteria.get(id), "label") || id).join(" · "); const approval = document.createElement("small"); approval.textContent = language === "ja" ? "合否値: 責任者未承認" : "Pass/fail values: owner approval pending"; name.append(nameText, criteria, approval); row.append(name); matrixRow.cells.forEach((matrixCell) => { const cell = document.createElement("td"); cell.id = matrixCell.system_requirement_id; cell.className = `infrastructure-demand demand-${matrixCell.demand_level}`; const identifier = document.createElement("code"); identifier.className = "system-requirement-id"; identifier.textContent = matrixCell.system_requirement_id; const status = document.createElement("span"); status.className = `system-requirement-evidence ${matrixCell.evidence_status}`; status.textContent = tr(matrixCell.evidence_status === "quantitative-reference-linked" ? "quantitativeReferenceLinked" : "qualitativeEvidenceOnly"); const level = document.createElement("span"); level.className = "infrastructure-demand-level"; level.textContent = tr(demandLevelKeys[matrixCell.demand_level]); const rationale = document.createElement("p"); rationale.textContent = localized(matrixCell, "rationale"); const metrics = document.createElement("p"); metrics.className = "system-requirement-metrics"; metrics.textContent = `${tr("acceptanceMetrics")}: ${matrixCell.acceptance_metric_ids.map((id) => acceptanceMetricLabels.get(id) || id).join(" · ")}`; const approvalStatus = document.createElement("small"); approvalStatus.className = "system-requirement-approval"; approvalStatus.textContent = `${tr("systemRequirementCandidate")}: ${tr("ownerApprovalPending")}`; const gap = document.createElement("details"); const gapSummary = document.createElement("summary"); gapSummary.textContent = tr("measurementGap"); const gapText = document.createElement("p"); gapText.textContent = localized(matrixCell, "measurement_gap"); gap.append(gapSummary, gapText); cell.append(identifier, status, level, rationale, metrics, approvalStatus); if (matrixCell.quantitative_requirement_ids.length) { const evidence = document.createElement("details"); const evidenceSummary = document.createElement("summary"); evidenceSummary.textContent = tr("quantitativeEvidence"); const list = document.createElement("ul"); matrixCell.quantitative_requirement_ids.forEach((id) => { const item = document.createElement("li"); const link = document.createElement("a"); link.href = `#${id}`; link.textContent = id; item.append(link); list.append(item); }); evidence.append(evidenceSummary, list); cell.append(evidence); } cell.append(gap); row.append(cell); }); infrastructureBody.append(row); });
  ...
  function renderRoadmapAvailabilityDialog() {
    ...
    const status = document.createElement("span"); status.className = `summary-status availability-${event.availability_status}`; status.textContent = tr(availabilityStatusKeys[event.availability_status]);
    ...
    appendMetaItem(meta, tr("availabilityStatus"), tr(availabilityStatusKeys[event.availability_status]));
