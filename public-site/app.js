(function () {
  ...
  const copy = {
    ja: {
      ...
      navOverview: "概要", navCatalog: "調査カタログ", navOperational: "ソフトウェア利用調査", navSearch: "検索", navRoadmaps: "ロードマップ", navScenarios: "システム整備計画案", navReports: "報告書", navGlossary: "専門用語",
      ...
      plannerCandidateKicker: "暫定構成算定", plannerCandidateTitle: "CPU・GPU複合計算基盤 構成算定", plannerCandidateLead: "公開提示価格と計画用仮定から概算する参照構成と、公式ロードマップ上の先行候補を分けて検討します。", openPlannerCandidate: "構成算定を開く",
    ...
    en: {
      ...
      navOverview: "Overview", navCatalog: "Research catalog", navOperational: "Software usage survey", navSearch: "Search", navRoadmaps: "Roadmaps", navScenarios: "System planning options", navReports: "Reports", navGlossary: "Glossary",
      ...
      plannerCandidateKicker: "PROVISIONAL CONFIGURATION PLANNING", plannerCandidateTitle: "CPU-GPU Accelerated Computing Infrastructure Planner", plannerCandidateLead: "Compare reference configurations estimated from public asking prices and explicit planning assumptions with forward-looking candidates on official roadmaps.", openPlannerCandidate: "Open the configuration planner",
  ...
  function referenceData() {
    return data.roadmap_reference_data || {terms: [], comparison_sets: []};
  }

  function referenceTermMap() {
    return new Map(referenceData().terms.map((term) => [term.term_id, term]));
  }

  function roadmapArtifactMap() {
    return new Map((data.roadmap_artifacts || []).map((roadmap) => [roadmap.roadmap_id, roadmap]));
  }

  function topicRoadmapIds(topic) {
    return new Set((topic?.related_roadmaps || []).map((roadmap) => roadmap.roadmap_id));
  }

  function relevantTermsForTopic(topic) {
    const roadmapIds = topicRoadmapIds(topic);
    return referenceData().terms.filter((term) => term.roadmap_ids.some((roadmapId) => roadmapIds.has(roadmapId)));
  }

  function currentTopic() {
    return data.topics.find((topic) => topic.topic_id === activeTopicId);
  }

  function aliasMatchesText(value, alias) {
    const lowerValue = String(value || "").toLocaleLowerCase();
    const lowerAlias = alias.toLocaleLowerCase();
    let start = lowerValue.indexOf(lowerAlias);
    while (start !== -1) {
      const end = start + lowerAlias.length;
      const asciiStart = /^[a-z0-9_]/i.test(alias);
      const asciiEnd = /[a-z0-9_]$/i.test(alias);
      const before = lowerValue[start - 1] || "";
      const after = lowerValue[end] || "";
      if ((!asciiStart || !/[a-z0-9_]/i.test(before)) && (!asciiEnd || !/[a-z0-9_]/i.test(after))) return true;
      start = lowerValue.indexOf(lowerAlias, start + 1);
    }
    return false;
  }

  function appendGlossaryText(root, value, topic = currentTopic()) {
    const entries = relevantTermsForTopic(topic)
    ...
    while ((match = matcher.exec(value)) !== null) {
      ...
      const asciiStart = /^[A-Za-z0-9_]/.test(alias);
  ...
  function referenceSource(sourceRef) {
    if (sourceRef.catalog_source_id) {
      const source = data.topic_decision_support.sources.find((entry) => entry.source_id === sourceRef.catalog_source_id);
      return source ? {roadmap: null, source} : null;
    }
    const roadmap = roadmapArtifactMap().get(sourceRef.roadmap_id);
    const source = roadmap?.sources.find((entry) => entry.source_id === sourceRef.source_id);
  ...
  function appendRoadmapLinks(root, roadmapIds, termId = null) {
    const artifacts = roadmapArtifactMap();
    roadmapIds.forEach((roadmapId) => {
      const roadmap = artifacts.get(roadmapId);
      if (!roadmap) return;
      const item = document.createElement("li");
      const link = document.createElement("a");
      const termQuery = termId ? `?term=${encodeURIComponent(termId)}&lang=${language}` : `?lang=${language}`;
      link.href = `roadmaps/${roadmap.slug}/${termQuery}`;
  ...
  function renderDecisionDimensions(root, profile, topic) {
    ...
    profile.hpci_decision_dimensions.forEach((dimension) => {
      ...
      appendGlossaryText(heading, localized(dimension, "label"), topic);
      const question = document.createElement("p");
      appendGlossaryText(question, localized(dimension, "question"), topic);
  ...
  function renderTechnologyItem(item, actorMap, topic) {
    ...
    const title = document.createElement("h5");
    appendGlossaryText(title, localized(item, "name"), topic);
    const badges = document.createElement("div");
    badges.className = "decision-badges";
    const stageBadgeKeys = {
      current: "stageCurrentBadge",
      "near-term": "stageNearTermBadge",
      research: "stageResearchBadge",
      contested: "stageContestedBadge"
    };
    [tr(stageBadgeKeys[item.stage]), enumLabel(item.maturity), `${tr("confidence")}: ${tr(item.confidence)}`].forEach((value) => {
    ...
    appendGlossaryText(timing, `${tr("timing")}: ${localized(item, "timing")}`, topic);
    const statement = document.createElement("p");
    statement.className = "decision-statement";
    appendGlossaryText(statement, localized(item, "statement"), topic);
    ...
    appendGlossaryText(relevance, localized(item, "hpci_relevance"), topic);
    ...
    localized(item, "adoption_conditions").forEach((condition) => {
      ...
      appendGlossaryText(entry, condition, topic);
  ...
  function renderDecisionSections(root, profile, actorMap, topic) {
    ...
    profile.sections.forEach((profileSection) => {
      const filtered = profileSection.items;
      if (!filtered.length) return;
      const section = document.createElement("section");
      section.className = "decision-technology-section";
      section.id = profileSection.section_id;
      const title = document.createElement("h3");
      appendGlossaryText(title, localized(profileSection, "title"), topic);
      const summary = document.createElement("p");
      summary.className = "decision-section-summary";
      appendGlossaryText(summary, localized(profileSection, "summary"), topic);
      ...
      stageOrder.forEach((stage) => {
        ...
        items.forEach((item) => {
          ...
          if (!previous) {
            ...
            list.appendChild(renderTechnologyItem(item, actorMap, topic));
            return;
          }
  ...
  function renderCoverageGaps(root, profile) {
    ...
    gaps.forEach((gap) => {
      ...
      const question = document.createElement("p");
      appendGlossaryText(question, localized(gap, "question"));
  ...
  function renderResearchHistory(root, summaries, openByDefault = false) {
    ...
    summaries.forEach((summary) => {
      ...
      appendGlossaryText(summaryText, localized(summary, "summary"));
      const meta = document.createElement("dl");
      ...
      summary.findings.forEach((finding) => {
        const item = document.createElement("li");
        const statement = document.createElement("p");
        appendGlossaryText(statement, localized(finding, "statement"));
      ...
      appendGlossaryText(caveatText, localized(summary, "caveat"));
  ...
  function topicReferenceText(topic, profile) {
    const values = [topic.title_ja, topic.title_en, topic.summary_ja, topic.summary_en];
    (topic.research_units || []).forEach((unit) => values.push(
      unit.title_ja, unit.title_en, unit.question_ja, unit.question_en
    ));
    if (profile) {
      values.push(profile.summary_ja, profile.summary_en);
      profile.hpci_decision_dimensions.forEach((dimension) => values.push(
        dimension.label_ja, dimension.label_en, dimension.question_ja, dimension.question_en
      ));
      profile.sections.forEach((section) => {
        values.push(section.title_ja, section.title_en, section.summary_ja, section.summary_en);
        section.items.forEach((item) => values.push(
          item.name_ja, item.name_en, item.statement_ja, item.statement_en,
          item.hpci_relevance_ja, item.hpci_relevance_en,
          ...(item.adoption_conditions_ja || []), ...(item.adoption_conditions_en || [])
        ));
      });
    }
  ...
  function matchedTopicTermIds(topic, profile) {
    const text = topicReferenceText(topic, profile);
    return new Set(relevantTermsForTopic(topic)
      .filter((term) => term.aliases.some((alias) => aliasMatchesText(text, alias)))
      .map((term) => term.term_id));
  }

  function topicComparisons(topic, profile) {
    const roadmapIds = topicRoadmapIds(topic);
    const termIds = matchedTopicTermIds(topic, profile);
    return referenceData().comparison_sets.filter((comparison) =>
      comparison.roadmap_ids.some((roadmapId) => roadmapIds.has(roadmapId)) &&
      comparison.rows.some((row) => termIds.has(row.term_id))
    );
  }

  function resolveTopicComparisons(topic, profile, comparisonRefs = null) {
    if (!comparisonRefs) {
      return topicComparisons(topic, profile).map((comparison) => ({comparison, termIds: null}));
    }
    const comparisons = new Map(referenceData().comparison_sets.map((comparison) => [comparison.comparison_id, comparison]));
    return comparisonRefs.map((reference) => ({
      comparison: comparisons.get(reference.comparison_id),
      termIds: new Set(reference.term_ids || [])
    })).filter((entry) => entry.comparison);
  }

  function renderTopicComparisons(root, topic, profile, options = {}) {
    const {
      comparisonRefs = null,
      embedded = false,
      instanceId = "",
    ...
    const comparisons = resolveTopicComparisons(topic, profile, comparisonRefs);
    if (!comparisons.length) return;
    const terms = referenceTermMap();
    ...
    comparisons.forEach(({comparison, termIds}) => {
      ...
      section.id = `catalog-comparison-${comparison.comparison_id}${instanceId ? `-${instanceId}` : ""}`;
      ...
      termHead.textContent = language === "ja" ? "技術" : "Technology";
      ...
      head.append(headRow);
      const body = document.createElement("tbody");
      comparison.rows.filter((row) => !termIds || termIds.has(row.term_id)).forEach((row) => {
        ...
        const itemStages = Array.from(new Set(profileItems
          .filter((profileItem) => linkedItemIds.has(profileItem.item_id))
          .map((profileItem) => profileItem.stage)));
        if (itemStages.length) {
          const stages = document.createElement("div");
          stages.className = "comparison-stage-badges";
          const labels = {
            current: "stageCurrentBadge", "near-term": "stageNearTermBadge",
            research: "stageResearchBadge", contested: "stageContestedBadge"
          };
          itemStages.forEach((stage) => {
            const badge = document.createElement("span");
            badge.className = `comparison-stage-badge comparison-stage-${stage}`;
      ...
      if (!embedded) {
        ...
        roadmapTitle.className = "comparison-roadmap-title";
        roadmapTitle.textContent = tr("comparisonRoadmaps");
        const roadmapLinks = document.createElement("ul");
        roadmapLinks.className = "related-roadmap-list comparison-roadmap-links";
        appendRoadmapLinks(roadmapLinks, comparison.roadmap_ids);
        section.append(roadmapTitle, roadmapLinks);
      }
  ...
  function renderTopicOverview(root, profile, topic) {
    ...
    overview.className = "topic-results-overview decision-overview structured-topic-overview";
    const heading = document.createElement("h3");
    heading.textContent = tr("topicOverview");
    const text = document.createElement("p");
    appendGlossaryText(text, localized(profile, "summary"), topic);
    const notice = document.createElement("p");
    notice.className = "decision-provisional-notice";
    notice.textContent = tr("provisionalNotice");
    overview.append(heading, text, notice);
    root.appendChild(overview);
  }

  function renderRelatedSurfaces(root, profile) {
    if (!profile.related_surface_ids.length) return;
    const heading = document.createElement("h3");
  ...
  function renderRoadmapReferences(root, roadmapIds, embedded = false) {
    if (!roadmapIds?.length) return;
    const section = document.createElement("section");
    section.className = embedded ? "unit-roadmaps" : "topic-related-roadmaps";
    const heading = document.createElement(embedded ? "h4" : "h3");
    heading.textContent = embedded ? tr("unitRoadmaps") : tr("relatedRoadmaps");
    const list = document.createElement("ul");
    list.className = "related-roadmap-list";
    appendRoadmapLinks(list, roadmapIds);
    section.append(heading, list);
    root.appendChild(section);
  }

  function renderRelatedTopics(root, topic) {
    if (!topic.related_topic_ids?.length) return;
    const section = document.createElement("section");
    section.className = "roadmap-related-topics topic-related-topics";
    const heading = document.createElement("h3");
    heading.textContent = tr("relatedTopics");
    section.append(heading, topicLinks(topic.related_topic_ids));
    root.appendChild(section);
  }
  ...
  function renderResearchUnitComponent(root, component, topic, profile, actorMap) {
    const unit = topic.research_units.find((entry) => entry.unit_id === component.unit_id);
    if (!unit) return;
    const profileSections = component.section_ids
      .map((sectionId) => profile.sections.find((section) => section.section_id === sectionId))
      .filter(Boolean);
    const profileItems = profileSections.flatMap((section) => section.items);
    const section = document.createElement("section");
    section.className = "structured-research-unit";
    section.id = `catalog-unit-${unit.unit_id}`;
    const header = document.createElement("header");
    header.className = "structured-research-unit-header";
    const heading = document.createElement("h3");
    heading.textContent = researchUnitLabel(topic, unit);
    const status = document.createElement("span");
    ...
    renderTopicComparisons(section, topic, profile, {
      ...
      embedded: true,
      instanceId: component.component_id,
      profileItems,
  ...
  function renderStructuredTopicLayout(root, topic, profile, summaries) {
    const actorMap = decisionActorMap();
    profile.page_layout.components.forEach((component) => {
      switch (component.type) {
        ...
        case "coverage-gaps": renderCoverageGaps(root, profile); break;
        case "research-history": renderResearchHistory(root, summaries, component.open_by_default === true); break;
        case "related-topics": renderRelatedTopics(root, topic); break;
        default: break;
      }
    });
  }
  ...
  function renderTopicDetail() {
    ...
    if (topic.topic_id === "CROSS-19") {
      ...
      const body = document.createElement("div");
      const title = document.createElement("h3");
      title.textContent = language === "ja" ? "ソフトウェア利用調査の公開結果" : "Published software usage survey";
      const lead = document.createElement("p");
    ...
    if (profile?.page_layout) {
      renderStructuredTopicLayout(root, topic, profile, summaries);
      return;
    }
    renderTopicScope(root, topic);
    if (profile) {
      renderTopicOverview(root, profile, topic);
      const actorMap = decisionActorMap();
      renderDecisionDimensions(root, profile, topic);
      renderDecisionSections(root, profile, actorMap, topic);
      renderRelatedSurfaces(root, profile);
      renderCoverageGaps(root, profile);
    }
    renderTopicComparisons(root, topic, profile);
  ...
  function renderTopicScope(root, topic, options = {}) {
    const {numbered = false} = options;
    if (!topic.summary_ja && !topic.research_units?.length) return;
    const section = document.createElement("section");
    section.className = `topic-scope${numbered ? " structured-unit-index" : ""}`;
    const heading = document.createElement("h3");
    heading.textContent = tr("scopeTitle");
    const summary = document.createElement("p");
    appendGlossaryText(summary, localized(topic, "summary"), topic);
    const list = document.createElement("ul");
    list.className = "research-unit-list";
    (topic.research_units || []).forEach((unit) => {
      const item = document.createElement("li");
      const title = numbered ? document.createElement("a") : document.createElement("strong");
      if (numbered) {
        title.href = `#catalog-unit-${unit.unit_id}`;
        title.textContent = researchUnitLabel(topic, unit);
        title.addEventListener("click", (event) => {
          event.preventDefault();
          document.getElementById(`catalog-unit-${unit.unit_id}`)?.scrollIntoView({block: "start"});
        });
      } else {
        appendGlossaryText(title, localized(unit, "title"), topic);
      }
      const status = document.createElement("span");
      status.className = "research-unit-status";
      status.textContent = tr(statusKeys[unit.status]);
      ...
      if (!numbered) unit.evidence_section_ids.forEach((id, index) => {
        const link = document.createElement("a");
        link.href = `#${id}`;
  ...
  function renderReports() {
    ...
    data.reports.forEach((report) => {
      ...
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.textContent = language === "ja" ? report.title : report.title_en;
      title.appendChild(link);
      const summary = document.createElement("p");
      summary.textContent = language === "ja" ? report.summary : report.summary_en;
      const meta = document.createElement("p");
