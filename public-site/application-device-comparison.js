(() => {
  ...
  function render(lang) {
    ...
    if (group) {
      ...
      root.append(unit);
      root.append(bars([...rows, ...forecasts.map(r => ({value: r.value, device: target.name + " ← " + rows.find(m => m.id === r.baseline_measurement_id).device, kind: "conditional"}))], local(group, "title") + " / " + group.unit, lang, t));
