...
(function (root) {
  ...
  function evaluate(request, product, model, architecture = {}) {
    ...
    candidate.cases = PRICE_CASES.map(priceCase => {
      ...
      const at = n => {
        ...
        const bom = physicalBom(n, blocks, {...model, defaults: d});
        ...
        const auxiliaryPower = bom.compute_nic_ports * pa.compute_nic_port_kw +
        ...
        const fits = bom.compute_fabric_supported && modules + p.bundled_nvme_drives <= p.local_nvme_bays &&
      ...
      if (v.n < 1 || !v.fits || demandConflict) {
        ...
        failure.reason = demandConflict ? "absolute-demand-exceeds-budget-or-manual-lock" : "budget-facility-drive-bay-or-manual-lock-conflict";
        return failure;
      }
  ...
  const api = {ENGINE_VERSION: "concept-estimate-0.2.0", evaluate, compatible, dateWindow, physicalBom, demandRequirements};
