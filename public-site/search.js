(function () {
  ...
  function searchItems() {
    ...
    (data.latest_updates?.updates || []).forEach((update) => items.push({type: "update", id: update.update_id, title_ja: update.title_ja, title_en: update.title_en, body_ja: update.summary_ja, body_en: update.summary_en, search: flatten(update), href: `${rootPrefix}updates/?lang=${language}#${encodeURIComponent(update.update_id)}`, status: update.consensus_status}));
    ...
    data.roadmap_reference_data.terms.forEach((term) => items.push({type: "term", id: term.term_id, title_ja: term.label_ja, title_en: term.label_en, body_ja: term.short_definition_ja, body_en: term.short_definition_en, search: flatten(term), href: `${rootPrefix}glossary/?term=${encodeURIComponent(term.term_id)}&lang=${language}#${encodeURIComponent(term.term_id)}`, status: data.roadmap_reference_data.consensus_status}));
    ...
    data.application_performance_overview.applications.forEach((application) => items.push({type: "application", id: application.application_id, title_ja: application.name, title_en: application.name, body_ja: application.domain_ja, body_en: application.domain_en, search: flatten(application), href: `${rootPrefix}analytics/application-performance/?lang=${language}&app=${encodeURIComponent(application.application_id)}#${encodeURIComponent(application.application_id)}`, status: data.application_performance_overview.consensus_status}));
