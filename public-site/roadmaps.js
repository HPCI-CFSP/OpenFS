(function () {
  ...
  const copy = {
    ja: {
      ...
      domainFilter: "分類で絞り込む", all: "すべて", domainHardware: "ハードウェア", domainSystemSoftware: "システムソフトウェア", domainApplications: "アプリケーション", domainCrossCutting: "分野横断", search: "検索", searchPlaceholder: "名称または分類", roadmapColumn: "ロードマップ", domainColumn: "分類", horizonColumn: "対象期間", researchAsOf: "調査基準日", researchStatus: "調査状況", coverageStatus: "調査範囲", consensusStatus: "合意判定状況", contentUpdated: "内容更新", updatedColumn: "内容更新", noRoadmaps: "条件に一致するロードマップはありません。",
      ...
      revisionKicker: "来歴", revisionTitle: "更新履歴と再現情報", artifactId: "成果物ID", sourceCommit: "公開データの基点コミット", closeDialog: "詳細を閉じる", footerDescription: "HPCI-CFSP 公開調査ビュー"
    },
    en: {
      ...
      revisionKicker: "PROVENANCE", revisionTitle: "Revision and reproducibility", artifactId: "Artifact ID", sourceCommit: "Public input commit", closeDialog: "Close details", footerDescription: "HPCI-CFSP public research view"
    }
  };
  ...
  function renderRoadmapIndex() {
    ...
    filtered.forEach((roadmap) => {
      ...
      const updated = document.createElement("td"); const time = document.createElement("time"); time.dateTime = roadmap.updated_at; time.textContent = formatJst(roadmap.updated_at); updated.append(time); row.append(title, domain, horizon, asOf, research, consensus, updated); root.append(row);
    });
