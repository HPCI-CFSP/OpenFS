(function () {
  ...
  const copy = {
    ja: {
      ...
      plannerCandidateKicker: "暫定構成算定", plannerCandidateTitle: "GPU集中型AI for Science構成算定 Candidate", plannerCandidateLead: "既存3案とは別の参照アーキテクチャを使い、入力条件に応じたNVIDIA案とAMD案をブラウザ内で比較します。Consensus Gateは未完了で、調達判断には使用できません。", openPlannerCandidate: "構成算定を開く",
    ...
    en: {
      ...
      plannerCandidateKicker: "PROVISIONAL CONFIGURATION PLANNING", plannerCandidateTitle: "GPU-Centric AI for Science Configuration Candidate", plannerCandidateLead: "Use a reference architecture separate from the three planning options to compare NVIDIA and AMD configurations locally from user inputs. The Consensus Gate is incomplete, and the result must not be used for procurement.", openPlannerCandidate: "Open the configuration planner",
