(function () {
  ...
  const copy = {
    ja: {
      ...
      navOverview: "概要", navCatalog: "調査カタログ", navSearch: "検索", navRoadmaps: "ロードマップ", navScenarios: "システム整備計画案", navReports: "報告書", navGlossary: "専門用語", footerDescription: "HPCI-CFSP 公開調査ビュー",
      ...
      scenarioKicker: "システム整備計画案", scenarioIndexTitle: "CPU・GPU複合計算基盤", scenarioIndexLead: "公開提示価格と計画用仮定から概算する参照構成と、公式ロードマップ上の先行候補を分けて検討します。",
      provisionalPlanner: "暫定構成算定", plannerCandidateKicker: "暫定構成算定", plannerCandidateTitle: "CPU・GPU複合計算基盤 構成算定", plannerCandidateLead: "基本条件だけを入力し、採用候補、算定可能な規模、根拠、不足情報、切替判断を確認します。", openPlannerCandidate: "構成算定を開く",
      notRanking: "推奨順位を示すものではありません", scenarioNotice: "単一のAIモデルが作成したCandidateです。価格、供給、施設条件と独立したAIモデルによる合意判定は未完了で、調達判断には使用できません。",
    ...
    en: {
      ...
      navOverview: "Overview", navCatalog: "Research catalog", navSearch: "Search", navRoadmaps: "Roadmaps", navScenarios: "System planning options", navReports: "Reports", navGlossary: "Glossary", footerDescription: "HPCI-CFSP public research view",
      ...
      scenarioKicker: "SYSTEM PLANNING OPTIONS", scenarioIndexTitle: "CPU-GPU Accelerated Computing Infrastructure", scenarioIndexLead: "Compare reference configurations estimated from public asking prices and explicit planning assumptions with forward-looking candidates on official roadmaps.",
      provisionalPlanner: "PROVISIONAL CONFIGURATION PLANNING", plannerCandidateKicker: "PROVISIONAL CONFIGURATION PLANNING", plannerCandidateTitle: "CPU-GPU Accelerated Computing Infrastructure Planner", plannerCandidateLead: "Enter only the basic conditions, then review candidate platforms, quantifiable scale, evidence gaps, and decision gates.", openPlannerCandidate: "Open the configuration planner",
      notRanking: "The options are not ranked", scenarioNotice: "This Candidate was prepared by one AI model. Price, supply, facility constraints, and consensus review by independent AI models remain incomplete; it must not be used for procurement decisions.",
  ...
  function renderEvidencePage() {
    ...
    renderMetric(metrics, tr("marketAvailabilityEvents"), evidence.summary.availability_event_count, `${evidence.summary.availability_confirmed} ${language === "ja" ? "確定" : "confirmed"} · ${evidence.summary.availability_announced_target} ${language === "ja" ? "公表目標" : "announced targets"} · ${evidence.summary.availability_timing_undisclosed} ${language === "ja" ? "時期未公表" : "undated"}`);
    ...
    const queueSummary = document.getElementById("gap-queue-summary"); queueSummary.replaceChildren(); [[tr("p0ExplicitQueries"), gapQueue.summary.p0_explicit_query_overrides], [tr("p0FallbackQueries"), gapQueue.summary.p0_generated_query_fallbacks], [tr("assignedConsensus"), gapQueue.summary.consensus_review], [tr("productionReady"), gapQueue.summary.ready_for_scheduled_discovery], [tr("p0EvidenceReview"), gapQueue.summary.p0_evidence_review_required], [tr("p0Measurement"), gapQueue.summary.p0_reproducible_measurement_required], [tr("p0Conformance"), gapQueue.summary.p0_conformance_test_required], [tr("p0Authority"), gapQueue.summary.p0_authority_confirmation_required], [tr("p0Consensus"), gapQueue.summary.p0_consensus_quorum_required]].forEach(([labelText, value]) => { const item = document.createElement("div"); item.append(makeCell("span", labelText), makeCell("strong", value)); queueSummary.append(item); });
