...
- `knowledge/public/gpu-planner-publication.json`: human-approved publication
  manifest for the provisional GPU-centric AI for Science configuration planner.
  It binds the exact public Candidate inputs by SHA-256 and preserves incomplete
  Consensus and prohibited procurement use.
...
The roadmap builder rejects mismatches between the portfolio and exports, broken track, source, dependency, or milestone references, dates outside the horizon, quarter labels without quarter-level evidence, malformed undated entries, and OpenFS provisional-plan events that are not HPCI evaluation or adoption gates. The comparison view intentionally uses only milestones marked `comparison_priority: key`; supporting detail remains on each roadmap page.
...
Before adding a Topic Summary, Consensus Receipt, Scenario, Report, or Candidate
planning interface to Pages:
...
Pushes do not deploy until `OPENFS_PAGES_ENABLED=true`. Manual `workflow_dispatch` is allowed so an administrator can test the first deployment. Candidate planning interfaces require a separately approved publication manifest, remain visibly provisional, and cannot be presented as accepted scenarios or procurement recommendations. A Topic row always opens a detail view; Topics without an approved summary show a clear pending state.
...
5. Check Japanese/English switching, the roadmap index and dedicated detail
   routes, desktop and mobile widths, and confirm that Candidate tools retain
   their approval, Consensus, privacy, and procurement-use notices.
