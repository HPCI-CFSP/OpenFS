...
3. One shared public legend classifies the timing basis of each key outcome as `Attained`, `Official plan`, `Provisional outlook`, or `Timing unconfirmed`. The renderer derives these states from `timing_basis`: observed, as-of-baseline, and standard-release evidence is attained; vendor, project, and policy targets are official plans; OpenFS provisional plans and synthesis are provisional outlooks; and no-public-date items remain timing-unconfirmed. This status never replaces the event's product, standard, research, evaluation, adoption, or maturity metadata, which remains available in the detail view.
