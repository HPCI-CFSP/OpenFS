...
[Hot Chips 2026の調査台帳](https://hpci-cfsp.github.io/OpenFS/conferences/hot-chips-2026/)では、プログラム全48件と参加組織からカタログの調査結果を参照できます。関連一次情報の確認、要旨のみの確認、プログラムのみの確認を区別し、すべて暫定結果として扱います。[会議を起点とした調査手順](docs/operations/conference-research.md)も参照してください。
...
| `knowledge/public/procurement-cost-register.json` | 公開契約額、仕様書の取得状況、調達範囲の対応関係、費用内訳の未配賦額 |
| `config/budget-planning.json` | 五つの予算上限と暫定的な配分方針。仮配分額は見積価格ではない |
| `docs/planning/procurement-cost-estimation.md` | 公開情報による価格調査、条件の統一、校正要件、数量・TCOの未確認事項 |
| `config/catalog-taxonomy.json` | 全ての有効な調査項目とロードマップを6つの公開分類へ割り当て、調査項目の表示コードを定める正本。Pagesの分類フィルタはこのファイルから生成する |
| `config/source-watch-registry.json` | 継続監視する公式ページ・分析サイト、利用上の制約、影響を受けるTopic・ロードマップ・モニターの対応表 |
...
新しい調査課題とモニターは、`config/research-baseline.json`から調査項目を選択します。`FSBASE-002`の改訂5では、既存の正規Topic IDを60件全て保持し、AIおよび実験連携・リアルタイム処理のニーズと評価を分けるため4件を追加しました。既存60件の内訳は、自動処理では削除しない初期項目30件、文部科学省の2022～2025年度の全26件のPDFから追加した27件、人からの指示に基づく3件です。現在は履歴を含め64件を保持し、後継を明記した退役項目24件を除く40件をPagesに表示します。過去の根拠や合意判定の参照IDは変更しません。`config/catalog-taxonomy.json`は全ての有効な調査項目とロードマップを6分類のいずれか1つへ割り当て、使わなくなった表示コードも再利用しないよう予約します。

[現行カタログと移行対応表](docs/research-baseline/current-catalog.md)は、日英の調査範囲定義と`config/catalog-migration.json`から生成します。各Topicに具体的な問い、進捗、根拠節への参照を持つ小項目を設け、統合によって未調査の範囲を完了扱いにしません。ストレージのデバイス・ソフトウェア・統合製品はアーキテクチャで一体的に調査します。プログラミング環境を統合し、ソフトウェアの保守継続性は残し、科学シミュレーション・AI・実験連携とリアルタイム処理の各領域でニーズと評価を対応付けます。[カタログ維持手順](docs/operations/catalog-maintenance.md)には、調査の担当範囲、旧リンク、継続調査の要件を記載しています。

人間が明示的に依頼した単一のAIモデルによる調査では、[調査単位ごとの暫定更新手順](docs/operations/provisional-research-updates.md)を使えます。入力コミットと先行更新のハッシュを固定し、根拠と資料確認記録を追加して競合を検知します。結果は暫定のままで、未確認事項の解消、Consensusの成立、無人運転の有効化は行えません。ストレージのロードマップ、調達台帳、AI評価の調査単位に、`DIR-900015`に基づく試行更新を含めています。

手動で管理する監視先一覧は、繰り返し確認する公式の索引・ニュースページと、個々の主張を支える根拠資料を分離します。`tools/build_source_catalog_map.py`は、現行の登録済み公開根拠からURLとTopic、ロードマップ、トラックの対応表を生成します。監視ページの変更は兆候にすぎません。自動処理による採用には、個別の一次資料による変更の確認と、該当する合意判定（Consensus Gate）の通過が必要です。`.github/workflows/emerging-topic-discovery.yml`は新規Topicの探索を毎日起動できますが、試行結果、セキュリティプロファイル、予算、合意判定能力の準備が完了するまではFail-closedで停止します。
...
SemiAnalysisと公開記事一覧は論点発見の入口、IEEE Roadmaps・IRDS・HIRは長期的な
技術見通しを調べる監視先として登録しています。[利用上の規則](docs/research-baseline/source-watch-and-evidence-map.md)
に従い、匿名で読める公開情報だけを使い、根拠を個別資料で確認します。分析や技術目標を
確定した製品発売予定には置き換えません。登録によって自動調査やConsensusが有効になるわけではありません。
