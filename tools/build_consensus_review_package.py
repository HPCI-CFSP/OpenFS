...
def shared_units() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "falsification_prompts_ja": ["P0がHPCI判断への影響ではなく情報の入手しやすさで決まっていないか。", "未公表事項を否定的事実として扱っていないか。", "Monitor無効中の割当を自動調査済みと誤表示していないか。", "1件の検索結果だけでGapを解消済みにしていないか。", "性能モデルの校正データを検証データとして再利用し、単一環境の誤差を一般化していないか。", "異なる入力・精度・測定境界のbenchmarkを同条件比較として扱い、失敗runや正当性不合格を除外していないか。", "利用統計に個別job、識別子、自由記述、小セル、差し引き可能な単独抑制が残っていないか。", "コンパイラごとに異なるfeature集合を分母にし、vendor文書を実行試験と同等に扱っていないか。"],
...
def build_manifest(root: Path, base_commit: str, created_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "independence_requirements": {
            ...
            "reviewer_rule_ja": "支持票には少なくとも3つの独立group、3つのorigin group、登録済みの3モデル系統、2プロバイダ、2つの異なるHarness Repositoryが必要。各実行のHarness commitも固定する。作成モデルと同じgroup、同一会話のfork、同一出力を共有したreviewerは独立票として数えない。",
