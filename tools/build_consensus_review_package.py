...
def shared_units() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "artifact_paths": [*ROADMAP_PATHS, "knowledge/public/audits/roadmap-gap-queue.json", "config/source-registry.json", "config/roadmap-gap-query-overrides.json", "config/role-permissions.json", "config/monitors/MON-MEMORY-001.json", "config/monitors/MON-GLOBAL-TECH-001.json", "config/monitors/MON-HPCI-CENTERS-001.json", "config/monitors/MON-FS-BASELINE-001.json", "schemas/roadmap-gap-queue.schema.json", "schemas/roadmap-gap-query-overrides.schema.json", "schemas/performance-model-card.schema.json", "schemas/benchmark-result-bundle.schema.json", "tools/build_roadmap_gap_queue.py", "tools/check_performance_model_card.py", "tools/check_benchmark_result_bundle.py", "docs/research-baseline/performance-model-validation.md", "docs/research-baseline/reproducible-benchmark-results.md", "proposals/benchmark-results/README.md", "tools/run_controller.py", "tools/prepare_weekly_cycle.py"],
            ...
            "falsification_prompts_ja": ["P0がHPCI判断への影響ではなく情報の入手しやすさで決まっていないか。", "未公表事項を否定的事実として扱っていないか。", "Monitor無効中の割当を自動調査済みと誤表示していないか。", "1件の検索結果だけでGapを解消済みにしていないか。", "性能モデルの校正データを検証データとして再利用し、単一環境の誤差を一般化していないか。", "異なる入力・精度・測定境界のbenchmarkを同条件比較として扱い、失敗runや正当性不合格を除外していないか。"],
