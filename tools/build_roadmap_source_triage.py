...
def build_triage(root: Path, config_path: Path, audit_path: Path) -> dict[str, Any]:
        ...
        entries.append(
            {
                ...
                "reviewed_at": review["reviewed_at"] if review_is_current else None,
    ...
    return {
        ...
        "caveat_ja": "HTTP到達性の警告と単一のAIモデルによる本文確認の有無をまとめた暫定記録です。自動監査が未実施の情報源も含みます。確認記録がない項目には確認日を付けません。主張全体の独立検証やConsensusによる受理を意味しません。",
        "caveat_en": "This provisional record distinguishes HTTP-audit warnings from single-model content checks, including sources not yet automatically audited. Entries without a matching review have no review date. It does not independently validate the full claim or indicate Consensus acceptance.",
        "summary": {
