...
def build(root: Path, generated_at: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "method_ja": "6ロードマップの全Coverage Gapを優先度順に収集し、既存Monitorまたは独立Consensusパッケージへ明示的に割り当てた。P0は毎週、P1は毎月、P2は四半期ごとの再調査を原則とする。",
        "method_en": "Collects every Coverage Gap from the six roadmaps, orders them by priority, and assigns each to an existing Monitor or the independent Consensus package. P0 is normally rechecked weekly, P1 monthly, and P2 quarterly.",
        "caveat_ja": "このキューは調査の割当であり、Gapが解消したことを示さない。Monitorが無効の項目は本番準備ゲートを通過するまで自動検索されない。",
        "caveat_en": "This queue assigns research; it does not establish that a Gap is resolved. Items assigned to disabled Monitors are not searched automatically until production-readiness gates pass.",
        "summary": {
