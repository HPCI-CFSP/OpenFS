...
FACILITY_NETWORK_EDGE = {
    "dependency_id": "XDEP-FACILITY-NET",
    "upstream_roadmap_id": "RM-HW-FACILITY",
    "downstream_roadmap_id": "RM-HW-INTERCONNECT",
    "relationship": "constrains",
    "criticality": "high",
    "basis": "evidence-backed",
    "statement_ja": "施設の電力、冷却、ラック、配線条件は、ネットワークの構成、伝送媒体、到達距離、光接続の導入範囲を制約する。",
    "statement_en": "Facility power, cooling, rack, and cabling conditions constrain network topology, media, reach, and the deployable scope of optical connectivity.",
    "decision_impact_ja": "ネットワーク方式を選定する前に、スイッチ・光部品を含むラック電力、冷却方式、配線経路、交換単位をセンター別に確認する。",
    "decision_impact_en": "Before selecting a network, verify per-center rack power including switches and optics, cooling mode, cable routes, and replacement units.",
    "risk_if_late_ja": "施設条件の確認が遅れると、トポロジー、光部品、ラック配置の再設計が必要になり、導入時期と費用の不確実性が増える。",
    "risk_if_late_en": "Late facility validation can force topology, optics, and rack-layout redesign, increasing schedule and cost uncertainty.",
    "source_ids": ["SRC-NET009", "SRC-NET010"],
    "source_dependency_ids": ["DEP-NET-FACILITY-001"],
    "gate_refs": ["MS-FACILITY-REQ-BASELINE", "MS-NET-HPCI-2027"],
    "coverage_gap_refs": ["GAP-FACILITY-001"],
}
...
def main():
    ...
    ] + [FACILITY_NETWORK_EDGE] + EDGES + REMAINING_EDGES
    ...
    payload["publication"] = {
        ...
        "human_approval_directive_id": "DIR-900105",
    }
